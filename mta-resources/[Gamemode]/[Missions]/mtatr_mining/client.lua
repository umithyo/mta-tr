...
function createMiningRock(x,y,z,rx,ry,rz)
	if (x) and (y) and (z) then
		...
		local blip = createBlipAttachedTo(rock,0,1.2,255,200,100,255,0,1000)
...
function createRocks(x,y,z) --Mining rocks (which some have in)
	if (x) and (y) and (z) then
		cleanupElementRocks()
	...
	if isElement(miningRocks[theRoc][1]) and attacker == localPlayer then
		...
		if miningRocks[theRoc][3] > 1000 then
			...
			if grams > vehicles[theVehid][1] then
				outputChatBox("Araç dolu. Devam etmek için topladığınız madeni satın.", 255, 0, 0) 
			return end
			local numm = math.random(1,7)
			if numm == 3 then
				outputChatBox("Bu maden çok sert, bomba yerleştirildi.", 255, 0, 0) 
				triggerServerEvent("mining.anim", resourceRoot,5)
...
function pickUp(hitElement,dim)
	if dim and hitElement == localPlayer and rocks[source] and not hasRock and not isPedInVehicle(localPlayer) then
		if not isElement(theVeh) then outputChatBox("Yeni bir araç oluşturun.", 255, 0, 0) return end
		local vx,vy,vz = getElementPosition(theVeh)
		local mx,my,mz = getElementPosition(localPlayer)
		if ( getDistanceBetweenPoints3D(vx,vy,vz,mx,my,mz) > 25 ) then outputChatBox("Aracın çok uzakta.", 255, 0, 0) return end 
		hasRock = true
		toggleAllControls(false)
		setPedWeaponSlot(localPlayer,0)
		lastCol = source
		triggerServerEvent("mining.anim", resourceRoot,2)
		timer5 = setTimer(function(col)
			...
			triggerServerEvent("mining.anim", resourceRoot,1)
		end,700,1,source)
		createVehMarker()
    triggerServerEvent("mining.freeze", resourceRoot, true)
	end
end
...
function createVehMarker()
	...
	if theVeh then
		...
		marker = createMarker(x,y,z,"cylinder",1,255,200,200)
...
function throwing(thePlayer)
	...
	triggerServerEvent("mining.anim", resourceRoot,4)
	for i,v in ipairs(ctrls) do
    ...
    timer1 = setTimer(
		function()
			...
			local gramN = math.random(200,450)*rocks[lastCol][3]
			grams = grams + math.ceil(gramN)
			triggerServerEvent("mining.freeze", resourceRoot, false, grams,theVehid)
			exports.mtatr_dx:drawStat("MinerID", "maden", grams.." Gram", 255, 200, 200)
			rocks[lastCol] = nil
			lastCol = nil
...
function onPlayerJobSwitch()
	local jobName = getElementData(localPlayer, "inmission")
	if (jobName == "Madenci") then
		if not hasMission then return end
		triggerServerEvent("mining.remaininggrams",resourceRoot,false,grams,theVehid)
		unloadMiningAreas()
		cleanupElementRocks()
		...
		hideTradingGUI()
		toggleAllControls(true)
		exports.mtatr_dx:drawStat("MinerID", "", "", 255, 200, 200)
	end
end
...
addEventHandler("onClientPlayerLeaveMission",localPlayer,onPlayerJobQuit)
...
addEventHandler ("onClientPlayerJoinMission", root, onTakeJob)

addEvent("mining.setGrams",true)
addEventHandler("mining.setGrams",root,function(gram)
	grams = gram
    exports.mtatr_dx:drawStat("MinerID", "maden", grams.." Gram", 255, 200, 200)
end)


addEvent("mining.triggerMission",true)
addEventHandler("mining.triggerMission",root,function(veh,id)
	if not ( veh == theVeh ) then 
		theVehid = id
		theVeh = veh 
		addEventHandler ("onClientElementDestroy", theVeh, onPlayerJobSwitch)
	end
	...
	loadMiningAreas()
    exports.mtatr_dx:drawStat("MinerID", "maden", grams.." Gram", 255, 200, 200)
	triggerServerEvent("mining.remaininggrams",resourceRoot,true,veh,id)
	if isElement(craftMarker1) then return end
	craftMarker1 = createMarker(615.31, 867.35, -43.96, "cylinder",4,255,200,200)
	addEventHandler("onClientMarkerHit",craftMarker1,showGUI) 
	craftMarker2 = createMarker(543.17, 880, -36.24, "cylinder",4,255,200,200)
...
function noGram()
	...
	triggerServerEvent("mining.remaininggrams",resourceRoot,false,0,vehid)
    exports.mtatr_dx:drawStat("MinerID", "maden", grams.." Gram", 255, 200, 200)
end	
...
local function resourceStart()
	local radarArea = createRadarArea(284,740,580, 250, 255, 0, 0, 150)
	local colShape = createColCuboid(284, 740,-50, 580, 270, 100)
	addEventHandler("onClientColShapeHit", colShape, zOnElementEnter)
	addEventHandler("onClientColShapeLeave", colShape, zOnElementExit)
end
addEventHandler("onClientResourceStart", resourceRoot, resourceStart)

function zOnElementEnter(hitElement, dimensionMatch)
	if not dimensionMatch then
		return
	end
	
	if hitElement == localPlayer then
		addEventHandler("onClientPlayerDamage", root, zManageDamage)
		addEventHandler("onClientVehicleDamage", root, zManageDamage)
		if not isElement (theVeh) then return; end
		for i, v in ipairs (getElementsByType"vehicle") do 
			setElementCollidableWith (theVeh, v, false);
			setElementCollidableWith (v, theVeh, false);
		end	
	end
end

function zOnElementExit(hitElement, dimensionMatch)
	if hitElement == localPlayer then
		removeEventHandler("onClientVehicleDamage", root, zManageDamage)
		removeEventHandler("onClientPlayerDamage", root, zManageDamage)
		if not isElement (theVeh) then return; end
		for i, v in ipairs (getElementsByType"vehicle") do 
			setElementCollidableWith (theVeh, v, true);
			setElementCollidableWith (v, theVeh, true);
		end	
	end
end

function zManageDamage(attacker)
	if isElement(attacker) and getElementType(attacker) ~= "ped" then
		cancelEvent()
	end
end