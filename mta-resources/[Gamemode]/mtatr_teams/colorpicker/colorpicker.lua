...
function colorPicker.create(id, start, title)
  ...
  if start then
    cp.color.h, cp.color.s, cp.color.v = rgb2hsv(unpack(start))
  end
  cp.gui = {}
  cp.gui.width = 416
  cp.gui.height = 304
  cp.gui.snaptreshold = 0.02
  cp.gui.window = _guiCreateWindow((sw-cp.gui.width)/2, (sh-cp.gui.height)/2, cp.gui.width, cp.gui.height, tostring(title or "COLORPICKER"), false)
  cp.gui.svmap = guiCreateStaticImage(16, 32, 256, 256, "files/blank.png", false, cp.gui.window)
  cp.gui.hbar = guiCreateStaticImage(288, 32, 32, 256, "files/blank.png", false, cp.gui.window)
  cp.gui.blank = guiCreateStaticImage(336, 32, 64, 64, "files/blank.png", false, cp.gui.window)
  ...
  cp.gui.okb = _guiCreateButton(336, 235, 64, 24, "OK", false, cp.gui.window)
  cp.gui.closeb = _guiCreateButton(336, 265, 64, 24, "Cancel", false, cp.gui.window)
  guiWindowSetSizable(cp.gui.window, false)	
  if isElement (gr.window[1]) then 
	guiMoveToBack (gr.window[1]) 
  end	
  cp.handlers = {}
...
function colorPicker:render()
  ...
  dxDrawImage(x+16, y+32, 256, 256, "files/sv.png", 0, 0, 0, self.color.white, self.gui.focus)
  dxDrawImage(x+288, y+32, 32, 256, "files/h.png", 0, 0, 0, self.color.white, self.gui.focus)
  dxDrawImageSection(x+8+math.floor(256*self.color.s), y+24+(256-math.floor(256*self.color.v)), 16, 16, 0, 0, 16, 16, "files/cursor.png", 0, 0, 0, self.color.white, true)
  dxDrawImageSection(x+280, y+24+(256-math.floor(256*self.color.h)), 48, 16, 16, 0, 48, 16, "files/cursor.png", 0, 0, 0, self.color.huecurrent, true)
