...
class Bot extends EventEmitter {
    ...
    setChannelStatus(status){
        ...
        return this.channel.setTopic(status);
    }
    ...
    _ready() {
        ...
        // for (let channel of this.client.channels.filter(ch => ch.type == "dm" || ch.type == "group")) {
        //     channel.delete();
        // }

        // // Leave ignored guilds
        // for (let guild of this.client.guilds.filter(g => g.id !== this.guildId)) {
        ...
        if (guild) {
            this.guild = guild;
            let channel = guild.channels.find(x => x.name == this.channelName);
            if (channel) {
    ...
    _guildCreate(guild) {
        ...
        if (guild.id !== this.guildId) {
            ...
            this.guild = guild;
            this.channel = guild.channels.find(x=>x.name == this.channelName) || null;
        }
    }
