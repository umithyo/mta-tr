...
let config = {
    ...
    passphrase: process.env.DISCORD_PASSPHRASE,
...
for (let botconfig of config.bots) {
    ...
    let bot = new Bot(config.guild, channel, config.token);
    channelBots.set(channel, bot);

    bot.on("ready", () => {
        console.log(`Bot ${bot.name} connected on ${channel}`);
        channelBots.set(channel, bot);
